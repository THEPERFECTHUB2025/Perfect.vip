...
do
	...
	do
		...
		function Library:Window(Options)
			...
			local NeonEffect = Instance.new('UIStroke', Main)
			local CornerEffect = Instance.new('UICorner', Main)
			...
			local BackgroundImage = Instance.new('ImageLabel', Main) -- Ajout de l'image de fond
			-- Configure BackgroundImage
			BackgroundImage.Name = "BackgroundImage"
			BackgroundImage.Size = UDim2.new(1, 0, 1, 0)
			BackgroundImage.Position = UDim2.new(0, 0, 0, 0)
			BackgroundImage.Image = "rbxassetid://128872190664421" -- ID de l'image
			BackgroundImage.BackgroundTransparency = 1
			BackgroundImage.ImageTransparency = 0.2 -- Ajuster la transparence pour la visibilité
			BackgroundImage.ZIndex = -1 -- Placer l'image derrière tout le reste
			--
			do -- Inserts
			...
			Main.Size = UDim2.new(0,600,0,450) -- Slightly larger for a modern look
			Main.BackgroundColor3 = Color3.fromRGB(20,20,20) -- Darker background
			Main.BackgroundTransparency = 1 -- Rendre le fond transparent pour voir l'image
			Main.BorderSizePixel = 0
			Main.AnchorPoint = Vector2.new(0.5,0.5)
			-- Add neon glow effect
			NeonEffect.Thickness = 2
			NeonEffect.Color = Color3.fromRGB(0,170,255) -- Neon blue
			NeonEffect.Transparency = 0.5
			-- Add rounded corners
			CornerEffect.CornerRadius = UDim.new(0,8) -- Adjusted to prevent corners from exceeding
			Library.Holder = Main
			--
			Inline.Name = "Inline"
			...
			Middle.BackgroundTransparency = 0.5 -- Ajout de la transparence
			Middle.BorderColor3 = Color3.new(0,0,0)
			Middle.BorderMode = Enum.BorderMode.Inset

			-- Ajout de l'image de fond pour le Middle
			local MiddleBackground = Instance.new('ImageLabel', Middle)
			MiddleBackground.Name = "Background"
			MiddleBackground.Size = UDim2.new(1, 0, 1, 0)
			MiddleBackground.Position = UDim2.new(0, 0, 0, 0)
			MiddleBackground.Image = "rbxassetid://128872190664421"
			MiddleBackground.BackgroundTransparency = 1
			MiddleBackground.ImageTransparency = 0.8
			MiddleBackground.ZIndex = -1
			--
			Line.Name = "Line"
			...
			Title.TextColor3 = Color3.fromRGB(0,255,170) -- Futuristic green
			Title.TextStrokeTransparency = 0.8 -- Slight glow effect
			...
			--
			Pages.Name = "Pages"
			Pages.Position = UDim2.new(0,80,0,0)
			...
			version.BackgroundColor3 = Color3.new(34, 255, 0)
			version.BackgroundTransparency = 1
			version.BorderSizePixel = 0
			version.BorderColor3 = Color3.new(0,0,0)
			version.Text = "<font color=\"#ff0000\">Perfect.vip    V3.0   @theperfectexploiter </font>"
		...
		function Library:Page(Options)
			...
			local NeonEffect = Instance.new('UIStroke', Holder)
			local CornerEffect = Instance.new('UICorner', Holder)
			...
			NeonEffect.Transparency = 0.5
			-- Add rounded corners
				CornerEffect.CornerRadius = UDim.new(0,6) -- Adjusted to prevent corners from exceeding
			--
			Button.Name = "Button"
			Button.Position = UDim2.new(0,0,0,3)
			Button.Size = UDim2.new(1,0,1,-2)
			Button.BackgroundColor3 = Color3.fromRGB(30,30,30) -- Darker for contrast
			...
			TopLine.BackgroundColor3 = Color3.new(1, 1, 1) -- Set to white for gradient
			local RainbowGradient = Instance.new('UIGradient', TopLine)
			...
			TopLine.ZIndex = 53
				-- Add real-time color change
			task.spawn(function()
				while task.wait(0.1) do
					local time = tick() % 5 / 5
					RainbowGradient.Rotation = time * 360
				end
			end)
			--
			Line.Name = "Line"
			...
			Title.TextColor3 = Color3.fromRGB(78, 162, 252) -- Keep "RAGE" text blue
			Title.FontFace = Library.UIFont
			Title.TextSize = Library.FontSize
			Title.ZIndex = 53
			Title.RichText = true
		...
		function Pages:Section(Options)
			...
			local NeonEffect = Instance.new('UIStroke', Frame)
			local CornerEffect = Instance.new('UICorner', Frame)
			...
			Frame.BackgroundColor3 = Color3.fromRGB(25,25,25) -- Slightly darker
			Frame.BorderSizePixel = 0
			-- Add neon glow effect
			NeonEffect.Thickness = 1
			NeonEffect.Color = Color3.fromRGB(0,255,170) -- Neon green
			NeonEffect.Transparency = 0.5
			-- Add rounded corners
			CornerEffect.CornerRadius = UDim.new(0,4) -- Adjusted to prevent corners from exceeding
			--
        ...
        function Sections:Toggle(Options)
            ...
            local Outline = Instance.new('UIStroke', Frame) -- Ajout du contour vert
        
            -- Configuration du contour
            Outline.Name = "Outline"
            Outline.Color = Color3.fromRGB(0, 255, 0) -- Couleur verte
            Outline.Thickness = 1
            Outline.Transparency = 1 -- Commence invisible
			...
			Holder.BackgroundTransparency = 1
			Holder.Text = ""
			Holder.TextColor3 = Color3.new(254,207,17)
			Holder.AutoButtonColor = false
			Holder.Font = Enum.Font.SourceSans
			Holder.TextSize = 14
			--
			...
			TextLabel.TextColor3 = Color3.fromRGB(257,207,17)
            ...
            local function SetState()
                ...
                if Toggle.Toggled then
                    Accent.Visible = true
                    Outline.Transparency = 0 -- Rendre visible quand activé
                    TextLabel.TextColor3 = Color3.fromRGB(254, 207, 17)
                else
                    Accent.Visible = false
                    Outline.Transparency = 1 -- Rendre invisible quand désactivé
                    TextLabel.TextColor3 = Color3.fromRGB(254, 207, 17)
                end
		...
		function Sections:List(Options)
                ...
                button.MouseButton1Click:Connect(function()
                    if MultiSelect then
                        if table.find(Selected, option) then
                            ...
                            text.TextColor3 = Color3.new(0.3059, 0.3059, 0.3059) -- Default color
                        else
                            table.insert(Selected, option)
                            text.TextColor3 = Color3.fromRGB(255, 255, 0) -- Yellow for selected
                        end
                        Library.Flags[Dropdown.Flag] = Selected
                        Dropdown.Callback(Selected)
                    else
                        for opt, tbl in next, Dropdown.OptionInsts do
                            if opt ~= option then
                                tbl.text.TextColor3 = Color3.new(0.3059, 0.3059, 0.3059) -- Default color
                            end
                        end
                        Selected = option
                        Value.Text = option
            ...
            function Dropdown:Set(option)
                if MultiSelect then
                    Selected = type(option) == "table" and option or {}
                    for opt, tbl in next, Dropdown.OptionInsts do
                        tbl.text.TextColor3 = table.find(Selected, opt) and Color3.fromRGB(255, 255, 0) or Color3.new(0.3059, 0.3059, 0.3059)
                    end
                    Library.Flags[Dropdown.Flag] = Selected
                    Dropdown.Callback(Selected)
                else
                    ...
                    if table.find(Dropdown.Options, option) then
                        Selected = option
                        Value.Text = option
                        Dropdown.OptionInsts[option].text.TextColor3 = Color3.fromRGB(255, 255, 0)
                        Library.Flags[Dropdown.Flag] = Selected
                        Dropdown.Callback(Selected)
                    else
                        Selected = nil
            ...
            function Dropdown:Refresh(tbl)
                local previousSelected = MultiSelect and table.clone(Selected) or Selected
                ...
                if MultiSelect then
                    for _, opt in ipairs(previousSelected) do
                        if table.find(tbl, opt) then
                            table.insert(Selected, opt)
                            Dropdown.OptionInsts[opt].text.TextColor3 = Color3.fromRGB(255, 255, 0) -- Yellow for selected
                        end
                    end
                elseif previousSelected and table.find(tbl, previousSelected) then
                    Selected = previousSelected
                    Value.Text = previousSelected
                    Dropdown.OptionInsts[previousSelected].text.TextColor3 = Color3.fromRGB(255, 255, 0) -- Yellow for selected
                end
        ...
        function Sections:UpdateList(flag, action)
            local dropdown = Flags[flag]
            if dropdown and dropdown.Refresh then
                if action.Add then
                    table.insert(dropdown.Options, action.Add)
                elseif action.Remove then
                    for i, option in ipairs(dropdown.Options) do
                        if option == action.Remove then
        ...
        function Sections:Whitelist(Targets)
            local Whitelist = {}
            local function applyWhitelist(target)
                -- Logique pour appliquer le whitelist sur la cible
                print("Applying whitelist to:", target.Name)
                -- ...existing code to apply whitelist...
            end
        
            local function removeWhitelist(target)
                -- Logique pour désactiver les actions sur la cible
                print("Removing whitelist from:", target.Name)
                -- ...code pour désactiver les actions comme forcehit, etc...
            end
        
            local function monitorWhitelist()
                while true do
                    for _, target in ipairs(Targets) do
                        if not Whitelist[target] or not target:IsDescendantOf(game) then
                            Whitelist[target] = true
                            applyWhitelist(target)
                        end
                    end
        
                    for target, _ in pairs(Whitelist) do
                        if not table.find(Targets, target) or not target:IsDescendantOf(game) then
                            Whitelist[target] = nil
                            removeWhitelist(target)
                        end
                    end
        
                    task.wait(1) -- Vérification toutes les secondes
                end
            end
		...
		function Sections:Multibox(Options)
            ...
            -- 
            function Dropdown:Refresh(tbl)
                local previousSelected = table.clone(Selected)
                content = table.clone(tbl)
                for _, opt in next, Dropdown.OptionInsts do
                    coroutine.wrap(function()
                        opt.button:Destroy()
