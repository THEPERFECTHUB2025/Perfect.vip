...
-- Cache pour les couleurs rainbow pour éviter les recalculs
local RainbowCache = {}
local LastRainbowUpdate = 0
local RainbowCacheDuration = 0.016 -- 60 FPS

-- Optimisation des fonctions mathématiques
...
-- Fonction rainbow optimisée avec cache
local function GetRainbowColor(offset)
    local currentTime = tick()
    
    -- Utilise le cache si la mise à jour est récente
    if currentTime - LastRainbowUpdate < RainbowCacheDuration then
        local cacheKey = MathFloor((currentTime * 50 + offset) % 360)
        if RainbowCache[cacheKey] then
            return RainbowCache[cacheKey]
        end
    end
    
    local hue = (currentTime * 50 + offset) % 360
    local color = FromHSV(hue / 360, 1, 1)
    
    -- Met en cache la couleur
    local cacheKey = MathFloor(hue)
    RainbowCache[cacheKey] = color
    LastRainbowUpdate = currentTime
    
    -- Nettoie le cache périodiquement
    if #RainbowCache > 360 then
        RainbowCache = {}
    end
...
-- Fonction pour créer un gradient rainbow optimisé
...
local MemoryManager = {
    ...
    AddThread = function(self, thread)
...
local Library do 
    ...
    Library = {
        ...
        RainbowEnabled = true,
        RainbowSpeed = 50,
        RainbowMode = "Smooth", -- "Smooth", "Pulse", "Wave", "Random", "Breathing"
        RainbowIntensity = 1.0,
        RainbowSaturation = 1.0,
        RainbowValue = 1.0,
        RainbowOffset = 0,
        RainbowPulseSpeed = 2.0,
        RainbowWaveAmplitude = 0.5,
        RainbowBreathingSpeed = 1.0,
        ...
        -- Système de thèmes rainbow dynamiques
        RainbowThemes = {
            ["Classic"] = {
                Saturation = 1.0,
                Value = 1.0,
    ...
    Library.Unload = function(self)
        -- Désactive les effets rainbow
        self.RainbowEnabled = false
        
        -- Nettoie le cache rainbow
        RainbowCache = {}
        
        -- Nettoie le gestionnaire de mémoire
        if self.MemoryManager then
            self.MemoryManager:Cleanup()
        end
        
        -- Déconnecte toutes les connexions
        for Index, Value in self.Connections do 
            if Value.Connection and typeof(Value.Connection) == "RBXScriptConnection" then
                Value.Connection:Disconnect()
            end
        end

        -- Ferme tous les threads
        ...
        -- Nettoie la liste des raccourcis
        if self.KeyList then
            -- Supprime la liste des raccourcis si elle existe
        end
    ...
    local Components = { } do
            ...
            local Items = { } do 
                ...
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        RainbowBorder.Instance.Color = GetRainbowColor(0)
                    end
                end)
                ...
                -- Animation du gradient rainbow
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        LinerGradient.Instance.Color = CreateRainbowGradient()
                        local GradientOffset = MathAbs(MathSin(tick() * Library.RainbowSpeed * 0.1))
                        LinerGradient.Instance.Offset = Vector2New(GradientOffset, 0)
                    end
                end)
            end
        ...
        Components.Page = function(Data)
            ...
            local Items = { } do 
                ...
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        Items["Inactive"].Instance.TextColor3 = GetRainbowColor(60)
                    end
                end)
                ...
                Items["Liner"] = Instances:Create("Frame", {
                    ...
                    ZIndex = 2,
                    BorderSizePixel = 0,
                    BackgroundColor3 = GetRainbowColor(120)
                })

                -- Animation de la ligne rainbow
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        Items["Liner"].Instance.BackgroundColor3 = GetRainbowColor(120)
                    end
                end)
                ...
                -- Animation du gradient de la ligne
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        LinerGradient.Instance.Color = CreateRainbowGradient()
                        local GradientOffset = MathAbs(MathSin(tick() * Library.RainbowSpeed * 0.12))
                        LinerGradient.Instance.Offset = Vector2New(GradientOffset, 0)
                    end
                end)
                ...
                Items["Glow"] = Instances:Create("Frame", {
                    ...
                    BorderSizePixel = 0,
                    BackgroundColor3 = GetRainbowColor(180)
                })

                -- Animation du glow rainbow
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        Items["Glow"].Instance.BackgroundColor3 = GetRainbowColor(180)
                    end
                end)
    ...
    Components.Colorpicker = function(Data)
        ...
        local Items = { } do
            ...
            local PagesBorder = Instances:Create("UIStroke", {
                ...
                Color = GetRainbowColor(240),
                LineJoinMode = Enum.LineJoinMode.Miter,
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                Thickness = 2
            })

            -- Animation de la bordure rainbow des pages
            Library:Connect(RunService.Heartbeat, function()
                if Library.RainbowEnabled then
                    PagesBorder.Instance.Color = GetRainbowColor(240)
                end
            end)
    ...
    Library.Window = function(self, Data)
        ...
        local Items = Components.Window({
            ...
            local TitleBorder = Instances:Create("UIStroke", {
                Parent = Items["Title"].Instance,
                Color = GetRainbowColor(120),
                LineJoinMode = Enum.LineJoinMode.Miter,
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                Thickness = 2
            })

            -- Animation de la bordure rainbow du titre
            Library:Connect(RunService.Heartbeat, function()
                if Library.RainbowEnabled then
                    TitleBorder.Instance.Color = GetRainbowColor(120)
                end
            end)
            ...
            Library:Connect(RunService.Heartbeat, function()
                if Library.RainbowEnabled then
                    Items["Text"].Instance.TextColor3 = GetRainbowColor(0)
                end
            end)
            ...
            if type(Window.GradientTitle) == "table" and Window.GradientTitle.Enabled then
                ...
                Library:Connect(RunService.Heartbeat, function()
                    if Library.RainbowEnabled then
                        TitleGradient.Instance.Color = CreateRainbowGradient()
            ...
            local ContentBorder = Instances:Create("UIStroke", {
                Parent = Items["Content"].Instance,
                Color = GetRainbowColor(180),
                LineJoinMode = Enum.LineJoinMode.Miter,
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                Thickness = 2
            })

            -- Animation de la bordure rainbow du contenu
            Library:Connect(RunService.Heartbeat, function()
                if Library.RainbowEnabled then
                    ContentBorder.Instance.Color = GetRainbowColor(180)
                end
            end)
        end
...
-- Fonctions pour contrôler les effets rainbow
...
function Library:GetRainbowColor(Offset)
    return GetRainbowColor(Offset or 0)
end
...
-- Nouvelles fonctions rainbow avancées
function Library:SetRainbowMode(Mode)
    if table.find(self.RainbowModes, Mode) then
        self.RainbowMode = Mode
    end
end

function Library:SetRainbowIntensity(Intensity)
    self.RainbowIntensity = MathClamp(Intensity, 0.1, 2.0)
end

function Library:SetRainbowSaturation(Saturation)
    self.RainbowSaturation = MathClamp(Saturation, 0.0, 1.0)
end

function Library:SetRainbowValue(Value)
    self.RainbowValue = MathClamp(Value, 0.0, 1.0)
end

function Library:SetRainbowOffset(Offset)
    self.RainbowOffset = Offset
end

function Library:SetRainbowPulseSpeed(Speed)
    self.RainbowPulseSpeed = MathClamp(Speed, 0.1, 5.0)
end

function Library:SetRainbowWaveAmplitude(Amplitude)
    self.RainbowWaveAmplitude = MathClamp(Amplitude, 0.0, 1.0)
end

function Library:SetRainbowBreathingSpeed(Speed)
    self.RainbowBreathingSpeed = MathClamp(Speed, 0.1, 3.0)
end

function Library:ApplyRainbowTheme(ThemeName)
    local theme = self.RainbowThemes[ThemeName]
    if theme then
        self:SetRainbowSaturation(theme.Saturation)
        self:SetRainbowValue(theme.Value)
        self:SetRainbowSpeed(theme.Speed)
    end
end

-- Fonction rainbow avancée avec différents modes
function Library:GetAdvancedRainbowColor(Offset)
    ...
    if self.RainbowMode == "Smooth" then
        hue = (time * self.RainbowSpeed + baseOffset + self.RainbowOffset) % 360
    elseif self.RainbowMode == "Pulse" then
        local pulse = MathSin(time * self.RainbowPulseSpeed) * 0.5 + 0.5
        hue = (time * self.RainbowSpeed * pulse + baseOffset + self.RainbowOffset) % 360
    elseif self.RainbowMode == "Wave" then
        local wave = MathSin(time * self.RainbowSpeed * 0.1 + baseOffset * 0.1) * self.RainbowWaveAmplitude
        hue = (time * self.RainbowSpeed + wave * 360 + self.RainbowOffset) % 360
    elseif self.RainbowMode == "Random" then
        hue = MathRandom(0, 360)
    elseif self.RainbowMode == "Breathing" then
        local breath = MathSin(time * self.RainbowBreathingSpeed) * 0.5 + 0.5
        hue = (time * self.RainbowSpeed * 0.5 + baseOffset + self.RainbowOffset) % 360
        -- Ajuste la valeur pour l'effet de respiration
        local adjustedValue = self.RainbowValue * (0.3 + breath * 0.7)
        return FromHSV(hue / 360, self.RainbowSaturation, adjustedValue)
    elseif self.RainbowMode == "Strobe" then
        local strobe = MathFloor(time * 10) % 2
        if strobe == 0 then
            return Color3.fromRGB(255, 255, 255)
        else
            hue = (time * self.RainbowSpeed + baseOffset + self.RainbowOffset) % 360
    ...
    return FromHSV(hue / 360, self.RainbowSaturation, self.RainbowValue)
end

-- Fonction pour créer un gradient rainbow avancé
function Library:CreateAdvancedRainbowGradient()
    return RGBSequence{
        RGBSequenceKeypoint(0, self:GetAdvancedRainbowColor(0)),
        RGBSequenceKeypoint(0.2, self:GetAdvancedRainbowColor(72)),
        RGBSequenceKeypoint(0.4, self:GetAdvancedRainbowColor(144)),
        RGBSequenceKeypoint(0.6, self:GetAdvancedRainbowColor(216)),
        RGBSequenceKeypoint(0.8, self:GetAdvancedRainbowColor(288)),
        RGBSequenceKeypoint(1, self:GetAdvancedRainbowColor(360))
    }
end

-- Fonction pour animer un élément avec des effets rainbow
function Library:AnimateWithRainbow(Element, Property, Offset)
    ...
    connection = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        local color = self:GetAdvancedRainbowColor(Offset or 0)
        if Element[Property] then
            Element[Property] = color
        end
    end)
    
    -- Ajoute la connexion au gestionnaire de mémoire
    self.MemoryManager:AddConnection(connection)
    
    return connection
end

-- Fonction pour créer un effet rainbow sur plusieurs éléments
function Library:CreateRainbowEffect(Elements, Properties, Offsets)
    local connections = {}
    
    for i, element in ipairs(Elements) do
        local property = Properties[i] or Properties[1] or "BackgroundColor3"
        local offset = Offsets and Offsets[i] or (i - 1) * 72
        
        local connection = self:AnimateWithRainbow(element, property, offset)
        if connection then
