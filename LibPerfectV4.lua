...
local Library do 
    ...
    Library.Window = function(self, Data)
        ...
        TimerLabel.Name = "ScriptTimer"
        TimerLabel.Parent = Items["Outline"].Instance
        TimerLabel.AnchorPoint = Vector2.new(0.5, 1)
        TimerLabel.Position = UDim2.new(0.5, 0, 1, -8)
        TimerLabel.Size = UDim2.new(0, 200, 0, 18)
        TimerLabel.BackgroundTransparency = 1
        TimerLabel.BorderSizePixel = 0
        TimerLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
        TimerLabel.Font = Enum.Font.SourceSans
        TimerLabel.TextSize = 14
        TimerLabel.Text = "Temps d'utilisation : 0s"
        TimerLabel.ZIndex = 1000

        spawn(function()
            while TimerLabel and TimerLabel.Parent do
                local elapsed = math.floor(os.clock() - LoadingTick)
                local min = math.floor(elapsed / 60)
                local sec = elapsed % 60
                if min > 0 then
                    TimerLabel.Text = string.format("Temps d'utilisation : %dm %02ds", min, sec)
                else
                    TimerLabel.Text = string.format("Temps d'utilisation : %ds", sec)
                end
                wait(1)
            end
        end)
...
-- Timer d'utilisation du script (affiché en bas au centre)
local TimerLabel = Instance.new("TextLabel")
TimerLabel.Name = "ScriptTimer"
TimerLabel.Parent = Items["Outline"].Instance
TimerLabel.AnchorPoint = Vector2.new(0.5, 1)
TimerLabel.Position = UDim2.new(0.5, 0, 1, -8) -- 8px au-dessus du bas
TimerLabel.Size = UDim2.new(0, 200, 0, 18)
TimerLabel.BackgroundTransparency = 1
TimerLabel.BorderSizePixel = 0
TimerLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
